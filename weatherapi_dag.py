...
def transform_load_data(task_instance):
    ...
    transformed_data = {"city": city,
                        ...
                        "minimun_temp_fahrenheit":min_temp_fahrenheit,
                        "maximum_temp_fahrenheit": max_temp_fahrenheit,
                        "pressure": pressure,
                        "humidity": humidity,
...
def save_joined_data_s3(task_instance):
    ...
    df = pd.DataFrame(data, columns = ['city', 'description', 'temperature_fahrenheit', 'feels_like_fahrenheit', 'minimun_temp_fahrenheit', 'maximum_temp_fahrenheit', 'pressure','humidity', 'wind_speed', 'time_of_record', 'sunrise_local_time', 'sunset_local_time', 'state', 'census_2020', 'land_area_sq_mile_2020'])
    # df.to_csv("joined_weather_data.csv", index=False)
    now = datetime.now()
    dt_string = now.strftime("%d%m%Y%H%M%S")
    dt_string = 'joined_weather_data_' + dt_string
    df.to_csv(f"s3://s3-parallel-qi/{dt_string}.csv", index=False)
...
with DAG('weather_dag_2',
        ...
        join_data = PostgresOperator(
                ...
                sql= '''SELECT 
                    ...
                    minimun_temp_fahrenheit,
                    maximum_temp_fahrenheit,
                    pressure,
                    humidity,
                    wind_speed,
                    time_of_record,
        ...
        with TaskGroup(group_id = 'group_a', tooltip= "Extract_from_S3_and_weatherapi") as group_A:
            ...
            uploadS3_to_postgres  = PostgresOperator(
                ...
                postgres_conn_id = "postgres_conn",
                sql = "SELECT aws_s3.table_import_from_s3('city_look_up', '', '(format csv, DELIMITER '','', HEADER true)', 's3-parallel-qi', 'us_city.csv', 'eu-north-1');"
            )
            ...
            create_table_2 = PostgresOperator(
                ...
                sql= ''' 
                    ...
                    minimun_temp_fahrenheit NUMERIC,
            ...
            is_houston_weather_api_ready = HttpSensor(
                ...
                endpoint='/data/2.5/weather?q=houston&APPID=6f16ba35be7775571a89418a9edf0040'
            )
